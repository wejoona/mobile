...
final providersListProvider = FutureProvider<List<ProviderData>>((ref) async {
  ...
  try {
    ...
    return providers.map((provider) {
      final code =
          provider['code'] as String? ?? provider['id'] as String? ?? '';
      return ProviderData(
        id: code,
        name: provider['name'] as String? ?? code,
        logo: provider['logo'] as String?,
        minAmount: (provider['minAmount'] as num?)?.toDouble(),
        maxAmount: (provider['maxAmount'] as num?)?.toDouble(),
        paymentMethodType: provider['paymentMethodType'] as String?,
        enumProvider: code,
      );
    }).toList();
  } catch (_) {
