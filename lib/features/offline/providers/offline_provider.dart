...
class OfflineNotifier extends Notifier<OfflineState> {
  ...
  Future<void> _processPendingTransfers() async {
    ...
    for (final transfer in transfersToProcess) {
      try {
        ...
        await sdk.transfers.createTransfer(
  ...
  Future<void> _refreshWalletData() async {
    ...
    // Cache beneficiaries
    final beneficiariesAsync = ref.read(beneficiariesProvider);
    beneficiariesAsync.whenData((beneficiaries) async {
      if (_cacheService != null) {
        await _cacheService!.cacheBeneficiaries(beneficiaries);
      }
    });
  }
